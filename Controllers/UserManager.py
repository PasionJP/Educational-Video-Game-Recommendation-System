...
class UserManager():
	...
	def signup(self, name, birthdate, email, password):
		user = self.dao.getByEmail(email)

		if user is not None:
			return "already_exists"

		user_info = {"name": name,"birthdate": birthdate, "email": email, "password": password}
	...
	def getGamesList(self, id):
		return self.game.getGamesByUser(id)

	def getUsersByGame(self, game_id):
		return self.dao.getUsersByGame(game_id)